...
namespace AircompanyTests.Tests
{
    ...
    public class AirportTest
    {
        ...
        [Test]
        public void When_HasMilitaryTransportPlanes_Expect_True()
        {
            ...
            {
                if ((militaryPlane.GetPlaneType() == MilitaryTypes.TRANSPORT))
                {
        ...
        [Test]
        public void IsPassengerPlane_WithMaxPassengerCapacity_NotNull()
        {
            Airport airport = new Airport(planes);
            PassengerPlane expectedPlaneWithMaxPassengersCapacity = airport.GetPassengerPlaneWithMaxPassengersCapacity();
            Assert.That(expectedPlaneWithMaxPassengersCapacity != null);
        }

        [Test]
        public void When_NextPlaneIsHigherThenCurrent_Expect_True()
        {
            Airport airport = new Airport(planes);
            airport = airport.SortByMaxLoadCapacity();
            List<Plane> planesSortedByMaxLoadCapacity = airport.GetPlanes().ToList();

            bool nextPlaneIsHigherThanCurrent = true;
            for (int i = 0; i < planesSortedByMaxLoadCapacity.Count - 1; i++)
            {
                Plane current = planesSortedByMaxLoadCapacity[i];
                Plane next = planesSortedByMaxLoadCapacity[i + 1];
                if (current.GetMaxLoadCapacity() > next.GetMaxLoadCapacity())
                {
                    nextPlaneIsHigherThanCurrent = false;
                }
            }
            Assert.That(nextPlaneIsHigherThanCurrent==true);
        }
    }
}
